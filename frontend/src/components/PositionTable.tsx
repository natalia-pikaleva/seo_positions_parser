...
export const PositionTable: React.FC<PositionTableProps> = ({
    ...
    const handleToggleArchive = async () => {
        try {
            ...
            // Toggle + получаем обновленную группу
            const updatedGroup = await archiveGroup(group.id);

            // Обновляем локально
            setEditableGroup(updatedGroup);
            onGroupLoaded(updatedGroup);

            // ✅ ОБНОВЛЯЕМ ПРОЕКТ ЦЕЛИКОМ
            if (refreshProject) {
    ...
    useEffect(() => {
        console.log('Props group изменился:', group.is_archived);
        if (!isClientView) {
            setEditableGroup(group);
        }
    }, [group.id, group.is_archived]); // ✅ Зависимости для перерендера
    ...
    return (
        ...
        <div className="space-y-6 flex flex-col h-[90vh]">
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
                {!isClientView && editableGroup.is_archived && (
                    <div className="mb-6 p-4 bg-gray-200 border border-gray-200 rounded-lg flex items-center gap-3">
                        ...
                        <div>
                            <p className="font-semibold text-gray-800">Группа в архиве</p>
                        </div>
                    </div>
                )}
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full md:w-auto">
                        {!isClientView && (
                            <>
                                ...
                                <button
                                    className={`px-4 py-2 rounded text-white transition-all ${editableGroup.is_archived ? 'bg-green-600 hover:bg-green-700' : 'bg-gray-600 hover:bg-gray-700'
                                        }`}
                                    onClick={handleToggleArchive}
                                    disabled={loading}
                                >
                                    {editableGroup.is_archived ? 'Разархивировать' : 'В архив'}
                                </button>
                ...
                <div className="flex-shrink-0 min-h-0 overflow-auto">
                    <ExcelLikeTableView
